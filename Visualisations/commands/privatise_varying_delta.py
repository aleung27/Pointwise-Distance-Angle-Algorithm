...
from common import PRESET_DELTAS, CustomScalarFormatter


def command_privatise_varying_delta(
    initial_gdf: gpd.GeoDataFrame, boundaries: gpd.GeoDataFrame
):
    """
    Conducts an experiment for a given input MMSI and epsilon value.
    Varies the delta value and privatises the route for each method
    ...
    # Run the privitisation against the different schemes available
    sdd_gdfs = [
    ...
    for i in range(len(PRESET_DELTAS)):
        ...
        ax.set_title(f"δ={PRESET_DELTAS[i]}", y=0.9)
    ...
    fig.suptitle(f"Privatisation of Ship {ship_id} Against Varying δ (ε: {epsilon})")
    fig.legend(
