...
from common import PRESET_EPSILONS, TEST_ITERATIONS


def command_error_varying_epsilon(
    initial_gdf: gpd.GeoDataFrame, boundaries: gpd.GeoDataFrame
):
    """
    Conducts an experiment for a given input MMSI and delta value.
    Varies the epsilon value and calculates the error for each method
    ...
    # For each method, run through each test value of epsilon TEST_ITERATIONS times
    for m in methods:
        res: Dict[str, List[Any]] = {"eps": [], "dtw": [], "dfd": []}
